...
function onErrorImage(e) {
    e.src = "//sgp1.vultrobjects.com/oneautobet/source/img/empty.jpg";
}
...
function callXMLHttpRequestMethod(method,url, data) {
    let result;
    const xhr = new XMLHttpRequest();
...
function callXMLHttpRequestUploadFile(method,url, data) {
    var datas = new FormData();
    let result;
    const file = data;
    datas.append("myFile", file);
    const xhr = new XMLHttpRequest();
    xhr.open(method, url, false);
    // xhr.setRequestHeader('Content-type', 'application/json');
    xhr.setRequestHeader('x-api-key', 'Bearer ' + localStorage.getItem('token'));
